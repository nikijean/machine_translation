...
class NMT(nn.Module):
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_hidden_unsqueeze_transpose = torch.transpose(dec_hidden_unsqueeze, 1, 2)
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden_unsqueeze_transpose)
        ...
        sm = nn.Softmax(1)
        alpha_t = sm(e_t)
        enc_hiddens_transpose = torch.transpose(enc_hiddens, 1, 2)
        alpha_t_unsqueeze = torch.unsqueeze(alpha_t, 2)
        a_t = torch.bmm(enc_hiddens_transpose, alpha_t_unsqueeze)
